...
from db.repository import (
    ...
    Run,
    Target,
    count_results_by_run,
    count_verdicts_by_run,
    create_prompt,
    create_result,
    create_run,
    create_target,
    delete_verdicts_by_run,
    delete_prompt,
    get_all_prompts,
    get_all_runs,
    get_all_targets,
...
class TestVerdicts:
    ...
    def test_count_and_delete_by_run(self, db: sqlite3.Connection):
        t = _make_target()
        create_target(db, t)
        p = _make_prompt()
        create_prompt(db, p)
        ...
        assert count_results_by_run(db, r.id) == 1
        assert count_verdicts_by_run(db, r.id) == 1
        assert delete_verdicts_by_run(db, r.id) == 1
        assert count_verdicts_by_run(db, r.id) == 0
...
class TestSeedValidation:
    ...
    def test_library_yaml_validates(self):
        ...
        assert len(entries) == 88  # 35 starter prompts + 53 A01 expansion prompts
    ...
    def test_seed_into_memory_db(self, db: sqlite3.Connection):
        ...
        assert len(all_prompts) == 88

        # Verify category counts
        assert len(get_prompts_by_owasp(db, "A01")) == 70  # original 17 + 53 expansion
