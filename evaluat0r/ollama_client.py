...
Stack.md:
  ...
  - evaluat0r only — hamm0r never calls Ollama
...
async def check_ollama_available(
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(5.0, connect=5.0, read=5.0, write=5.0, pool=5.0)) as client:
            ...
            # Graceful fallback: use any locally installed model so analysis can run.
            if model_ids:
                return {
                    "available": True,
                    "model": model_ids[0],
                    "error": None,
                    "warning": (
                        f"Requested model '{model}' not found. "
                        f"Using installed model '{model_ids[0]}' instead."
                    ),
                }
