...
def render_html(data: ReportData) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "jinja2 is required for HTML/PDF reports. "
            "Install evaluat0r dependencies to use render_html/generate_pdf."
        ) from exc
...
def _md_escape(value: str) -> str:
    """Escape markdown table delimiters and keep line-breaks readable."""
    return (value or "").replace("|", "\\|").replace("\n", "<br>")


def render_markdown(data: ReportData) -> str:
    """Render a markdown security report from ReportData."""
    lines: list[str] = []
    lines.append("# AI Security Assessment Report")
    lines.append("")
    lines.append("## Executive Summary")
    lines.append(f"- Engagement: {data.engagement_name}")
    lines.append(f"- Target: {data.target_name} ({data.target_url})")
    lines.append(f"- Tester: {data.tester_name}")
    ...
    lines.append(f"- Evaluated Results: **{data.total_evaluated}/{data.total_prompts}**")
    lines.append(
        "- Verdict Split: "
        f"SUCCESS {data.success_count}, PARTIAL {data.partial_count}, "
        f"FAIL {data.fail_count}, UNCLEAR {data.unclear_count}"
    )
    lines.append("")

    lines.append("## OWASP Category Breakdown")
    if data.categories:
        lines.append("| OWASP | Category | Total | Success | Partial | Fail | Unclear |")
        lines.append("|---|---|---:|---:|---:|---:|---:|")
        for cat in data.categories:
            lines.append(
                f"| {cat.code} | {_md_escape(cat.name)} | {cat.total} | {cat.success} | "
                f"{cat.partial} | {cat.fail} | {cat.unclear} |"
            )
    else:
    ...
    if not data.findings:
        lines.append("_No successful or partial attacks detected in this run._")
        lines.append("")
        return "\n".join(lines)

    for i, finding in enumerate(data.findings, start=1):
        lines.append(f"### Finding {i}: {finding.verdict} — {finding.owasp_ref} ({finding.category})")
        lines.append(f"- Prompt ID: `{finding.prompt_id}`")
        lines.append(f"- Severity: {finding.severity}")
        lines.append(f"- Confidence: {round(finding.confidence, 2)}")
        lines.append(f"- Reason: {_md_escape(finding.reason)}")
        lines.append("")
        lines.append("**Prompt**")
        lines.append("")
        lines.append("```text")
        lines.append(finding.prompt_text or "")
        lines.append("```")
        lines.append("")
        lines.append("**Response**")
        lines.append("")
        lines.append("```text")
        lines.append(finding.response_text or "")
        lines.append("```")
        lines.append("")
...
def generate_markdown(db: sqlite3.Connection, run_id: str, output_path: str) -> str:
    """Generate a markdown report for a run and return the output path."""
    data = build_report_data(db, run_id)
