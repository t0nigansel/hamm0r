...
Nothing in core may `import` from `evaluat0r/` or depend on a model runtime.

## Modules

- `promt0r` — UI + runner + sidecar command layer (core)
- `evaluat0r` — judge + report generator (analyzer, separate module)
...
- Tauri + HTML/JS/CSS (vanilla, no framework)
...
- `runner/` — async attack/scenario execution (core)
- `sidecar/` — command handlers and dev-server bridge (core)
- `ui/` — frontend (core)
- `evaluat0r/` — judge + report tooling (analyzer)
- `prompts/` — bundled starter attack library, copied into the user
  folder on first run
- `storage/` — file I/O layer for prompts, requests, engagements
...
- [ ] Tests present and passing (pytest)
...
1. **Core must work without the analyzer.** No import from `evaluat0r/` in
   core modules. No assumed model at runtime in the default flow.
...
8. **Sidecar never talks to the LLM directly.** Sidecar calls runner,
   runner calls the target. This layering is deliberate.
...
- No `async def` without reason — if there is no `await`, it's synchronous
...
- `python -m sidecar.dev_server --engagement ~/hamm0r/engagements/demo --port 9274` —
  start dev server against a specific engagement folder
- `python -m runner --engagement ~/hamm0r/engagements/demo --request my-target --prompt-set owasp-llm` —
  runner as CLI
- `python -m evaluat0r --engagement ~/hamm0r/engagements/demo --run latest --output report.pdf` —
  generate report (analyzer only)
- `python scripts/seed_prompts.py --dest ~/hamm0r/prompts` — seed the
  bundled starter library into the user folder
- `pytest` — all tests
- `pytest tests/eval/` — eval suite only (golden tasks)
- `ruff check .` — linting
- `mypy .` — types
